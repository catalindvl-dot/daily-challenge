...
export default function ChallengeIntro({ today }: ChallengeIntroProps) {
  ...
  return (
    <main className="relative flex flex-1 items-start justify-center overflow-hidden px-6 pb-8 pt-32 [@media(max-height:900px)]:pt-20 [@media(max-height:760px)]:pt-12">
      ...
      <Container
        className={`relative max-w-3xl translate-y-12 text-center transition-all duration-300 [@media(max-height:900px)]:translate-y-6 [@media(max-height:760px)]:translate-y-2 ${isVisible ? "opacity-100" : "opacity-0"
          }`}
      >
        <div>
